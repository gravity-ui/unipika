const tagged = require('../plugins/tagged');
...
(function () {
    ...
    function wrapScalar(node, settings, formattedValue) {
        ...
        if (validateCategory(node.$category)) {
            className +=
                WHITESPACE + 'pg_category_' + /*'unipika-' + */ node.$category.toLowerCase();
        }
    ...
    function wrapOptional(node, settings, formattedValue) {
        const isWrappableNode = node.$value === null || node.$type === 'yql.tagged';

        const needWrap = node.$optional > 0 && isWrappableNode;
