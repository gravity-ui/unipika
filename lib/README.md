...
|---|---|---|---|
|validateDomain|`(domain: string) => boolean`|`() => false`|Функция, которая принимает на вход источник для типов `TaggedType` и возвращает, разрешено с него скачивание или нет. В случае если параметр не будет передан, типы `TaggedType` (картинки, аудио, видео) будут отформатированы как `StructType` |
