(function () {
    ...
    function convertSimpleType(data, dataType) {
        var type = wrapYQLType(dataType.toLowerCase());
    ...
    function convertTaggedType(tag, dataType, data, converter, validateDomain) {
        var convertedValue = converter(data, dataType);
        var convertedStruct;
        switch (tag) {
            case 'url':
                if (dataType[0] === 'StructType') {
                    convertedStruct = convertStructToJSON(convertedValue);
                    if (convertedStruct.hasOwnProperty('href') && typeof convertedStruct.href === 'string') {
                        convertedValue = {
                            $type: 'tag_value',
            ...
            case 'videourl':
                if (dataType[0] === 'StructType') {
                    convertedStruct = convertStructToJSON(convertedValue);
                    if (
                        convertedStruct.hasOwnProperty('src') && 
                        typeof convertedStruct.src === 'string' &&
                        validateDomain(convertedStruct.src)
                    ) {
                        convertedValue = {
                            $type: 'tag_value',
                            $value: convertedStruct
    ...
    function convert(node, settings, flags) {
        ...
        function yqlToYson(data, dataType) {
            ...
            var isIncomplete, isBinary;
            if (data && Object.hasOwnProperty.call(data, 'val')) {
            ...
            switch (typeName) {
                case 'OptionalType': {
                    var hasData = (Array.isArray(data) && data.length);
                    var optionalData = hasData ?
                ...
                case 'TaggedType':
                    return convertTaggedType(dataType[1], dataType[2], data, yqlToYson, settings.validateDomain);
                ...
                case 'StructType':
                    var structData = data
                        .map(function (subData, index) {
                            var struct = typeValue[index];
                            var value = yqlToYson(subData, struct[1]);
                            if (settings.omitStructNull && value.$value === null) {
                                return null;
                            }

                            var key = convertSimpleType(struct[0], 'String');
                            key.$key = true;
                            return [
                                key,
                                value
                ...
                case 'VariantType':
                    var variantTypes = convertVariantTypes(typeValue);
                ...
                case 'DataType':
                    switch (typeValue) {
                        case 'String':
                        case 'Utf8': {
                            var normalizedValue = convertSimpleType(data, typeValue);
                            if (isBinary) {
                        ...
                        case 'Yson': {
                            var normalizedYsonValue = convertSimpleType(data, typeValue);
        ...
        return yqlToYson.apply(null, node);
    }

    function normalizeSettings(settings) {
        var normalizedSettings = settings || {};
        var validateDomain = settings && settings.validateDomain ? settings.validateDomain : () => false
        normalizedSettings.validateDomain = validateDomain
