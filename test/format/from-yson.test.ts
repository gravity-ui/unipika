...
describe('format', function () {
    var _formatFromYSON = unipika.formatFromYSON;
            ...
            describe('Gives correct output for: ' + JSON.stringify(input), function () {
                Object.keys(output).forEach(function (format) {
                    it('Format - ' + format, function () {
                        assert.strictEqual(output[format].plain, testUtils.toPlainText(_formatFromYSON, input, format, settings));
                        assert.strictEqual(output[format].html, testUtils.toHTMLText(_formatFromYSON, input, format, settings));
                    });
                });
            });
        });
    ...
    describe('formatFromYSON', function () {
        it('Exports', function () {
            assert.isDefined(_formatFromYSON);
        });

        it('isFunction', function () {
            assert.isFunction(_formatFromYSON);
        });
        ...
        describe('Miscellaneous', function () {
            it('Unescape keys in yson', function () {
                var settings = {
                    format: 'yson',
                    indent: 0,
                    decodeUTF8: false,
            ...
            it('Correct indentation yson', function () {
                var settings = {
                    format: 'yson',
                    indent: 4,
                    break: true,
            ...
            it('Valid JSON that can be parsed with JSON.parse', function () {
                ...
                function tryParsingAsJSON(value) {
                    var serialized = _formatFromYSON(value, settings);
